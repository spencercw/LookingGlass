...
static bool vulkan_pickPhysicalDevice(struct Inst * this)
{
  ...
  for (uint32_t i = 0; i < deviceCount; ++i)
  {
    ...
    {
      if ((queueFamilyProperties[j].queueFlags & requiredQueueFlags) == requiredQueueFlags)
      {
...
static bool vulkan_createDesktopDescriptorSetLayout(struct Inst * this)
{
...
static bool vulkan_renderStartup(LG_Renderer * renderer, bool useDMA)
{
  ...
  if (!vulkan_createDesktopDescriptorSetLayout(this))
    goto err_sampler;
  ...
  this->pipelineLayout = vulkan_createPipelineLayout(this->device,
  ...
  if (!vulkan_cursorInit(&this->cursor, &this->memoryProperties, this->device,
      this->commandBuffer, this->descriptorPool))
...
static void vulkan_postRenderBarrier(struct Inst * this, VkImage swapchainImage)
{
  struct VkImageMemoryBarrier swapchainImageBarrier =
  {
    .sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER,
    .pNext = NULL,
...
{
  VkImage swapchainImage = this->swapchainImages[imageIndex];
  ...
  if (!vulkan_cursorRender(this->cursor, this->swapchainImageViews[imageIndex],
      (this->format.rotate + rotate) % LG_ROTATE_MAX, this->width,
      this->height))
    return false;

  vulkan_postRenderBarrier(this, swapchainImage);
