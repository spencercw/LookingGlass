...
static bool vulkan_createDesktopUniformBuffer(struct Inst * this)
{
  ...
  VkResult result = vkCreateBuffer(this->device, &createInfo, NULL,
      &this->desktopUniformBuffer);
  if (result != VK_SUCCESS)
  {
    DEBUG_ERROR("Failed to create desktop uniform buffer (VkResult: %d)",
        result);
    goto err;
  }

  struct VkMemoryRequirements memoryRequirements;
  vkGetBufferMemoryRequirements(this->device, this->desktopUniformBuffer,
      &memoryRequirements);
  uint32_t memoryTypeIndex = vulkan_findMemoryType(this,
      memoryRequirements.memoryTypeBits,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT |
  ...
  {
    DEBUG_ERROR(
        "Could not find suitable memory type for desktop uniform buffer");
    goto err_buffer;
  }
  ...
  result = vkAllocateMemory(this->device, &allocateInfo, NULL,
      &this->desktopUniformBufferMemory);
  if (result != VK_SUCCESS)
  {
    DEBUG_ERROR(
        "Failed to allocate desktop uniform buffer memory (VkResult: %d)",
        result);
    goto err_buffer;
  }

  result = vkBindBufferMemory(this->device, this->desktopUniformBuffer,
      this->desktopUniformBufferMemory, 0);
  if (result != VK_SUCCESS)
  {
    DEBUG_ERROR("Failed to bind desktop uniform buffer memory (VkResult: %d)",
        result);
    goto err_memory;
  }

  result = vkMapMemory(this->device, this->desktopUniformBufferMemory, 0,
      VK_WHOLE_SIZE, 0, &this->desktopUniformBufferMap);
  if (result != VK_SUCCESS)
  {
    DEBUG_ERROR("Failed to map desktop uniform buffer memory (VkResult: %d)",
        result);
    goto err_memory;
  }
...
{
  struct VkBufferMemoryBarrier desktopUniformBufferBarrier =
  {
  ...
  vkCmdPipelineBarrier(this->commandBuffer, VK_PIPELINE_STAGE_HOST_BIT,
      ...
      &desktopUniformBufferBarrier, 1, &desktopImageBarrier);
