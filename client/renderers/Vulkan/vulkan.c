...
#include "quad_fullscreen.vert.spv.h"
#include "quad_transform_texture.vert.spv.h"
...
struct Inst
{
  ...
  VkDevice              device;
  VkQueue               queue;
  VkShaderModule        quadFullScreenVertexShader;
  VkShaderModule        quadTransformTextureVertexShader;
  ...
  Vulkan_ImGui   * imGui;
  //todo could these go inside Vulkan_ImGui?
  VkImage          imGuiImage;
...
static void vulkan_deinitialize(LG_Renderer * renderer)
{
  ...
  if (this->quadTransformTextureVertexShader)
    vkDestroyShaderModule(this->device, this->quadTransformTextureVertexShader,
        NULL);

  if (this->quadFullScreenVertexShader)
    vkDestroyShaderModule(this->device, this->quadFullScreenVertexShader, NULL);
...
static bool vulkan_createRenderPass(struct Inst * this)
{
  ...
  struct VkAttachmentDescription attachments[] =
  {
    ...
    {
      .flags = 0,
      .format = VK_FORMAT_B8G8R8A8_UNORM,//todo srgb?
      .samples = VK_SAMPLE_COUNT_1_BIT,
      .loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR,
      .storeOp = VK_ATTACHMENT_STORE_OP_STORE,//todo don't care
      .stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE,
      .stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE,
      .initialLayout = VK_IMAGE_LAYOUT_UNDEFINED,
...
static bool vulkan_createDesktopPipeline(struct Inst * this)
{
  ...
  this->pipeline = vulkan_createGraphicsPipeline(this->device,
      this->quadTransformTextureVertexShader, this->fragmentShader, NULL,
...
static void vulkan_calc_mouse_size(struct Inst * this)
{
  ...
  switch((this->format.rotate + this->rotate) % LG_ROTATE_MAX)
  {
    case LG_ROTATE_0:
    case LG_ROTATE_180:
      vulkan_cursorSetSize(this->cursor,
...
static bool vulkan_renderStartup(LG_Renderer * renderer, bool useDMA)
{
  ...
  this->quadFullScreenVertexShader = vulkan_loadShader(this->device,
      b_shader_quad_fullscreen_vert_spv,
      b_shader_quad_fullscreen_vert_spv_size - 1);
  if (!this->quadFullScreenVertexShader)
    goto err_device;

  this->quadTransformTextureVertexShader = vulkan_loadShader(this->device,
      b_shader_quad_transform_texture_vert_spv,
      b_shader_quad_transform_texture_vert_spv_size - 1);
  if (!this->quadTransformTextureVertexShader)
    goto err_quad_fullscreen_vert_shader;

  this->fragmentShader = vulkan_loadShader(this->device,
      b_shader_basic_frag_spv, b_shader_basic_frag_spv_size - 1);
  if (!this->fragmentShader)
    goto err_quad_transform_texture_vert_shader;
  ...
  if (!vulkan_cursorInit(&this->cursor, &this->memoryProperties, this->device,
      this->quadTransformTextureVertexShader, this->commandBuffer,
  ...
  if (!vulkan_imGuiInit(&this->imGui, this->instance, this->physicalDevice,
      ...
      this->queue, this->quadFullScreenVertexShader, this->commandBuffer,
...
err_quad_transform_texture_vert_shader:
  vkDestroyShaderModule(this->device, this->quadTransformTextureVertexShader,
      NULL);
  this->quadTransformTextureVertexShader = NULL;

err_quad_fullscreen_vert_shader:
  vkDestroyShaderModule(this->device, this->quadFullScreenVertexShader, NULL);
  this->quadFullScreenVertexShader = NULL;
