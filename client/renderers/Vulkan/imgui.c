    ...
    VkDescriptorPool descriptorPool, VkFence fence)
{
  *this = calloc(1, sizeof(Vulkan_ImGui));
  ...
  if (!createDescriptorSetLayout(*this, sampler))
    goto err_imgui;
    ...
    VkImage image, uint32_t width, uint32_t height)
{
  ...
  struct VkBufferMemoryBarrier bufferBarrier =
  {
  ...
  vkCmdPipelineBarrier(this->commandBuffer, VK_PIPELINE_STAGE_HOST_BIT,
      VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, NULL, 1, &bufferBarrier, 1,
      &preImageBarrier);
    ...
    uint8_t * data)
{
  ...
  vkUpdateDescriptorSets(this->device, 1, descriptorWrites, 0, NULL);
